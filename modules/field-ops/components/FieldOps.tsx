...
import { 
  ...
  CloudLightning, CheckCircle, Smartphone, AlertOctagon,
  ChevronLeft, Trash2, Sparkles, RefreshCw
} from 'lucide-react';
...
const FieldOps: React.FC<FieldOpsProps> = ({ projects, onSubmit }) => {
  ...
  const handleVoiceInput = async () => {
    // Note: In a real app, we'd use Web Speech API to get text. 
    // Here we simulate the AI processing a voice transcript.
    setIsListening(true);
    setTimeout(async () => {
    ...
    return (
      <div className="max-w-md mx-auto h-full flex flex-col p-4 space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <MenuButton icon={<Smartphone size={32}/>} label="سجل يومي" color="bg-slate-800" onClick={() => {}} />
        </div>
